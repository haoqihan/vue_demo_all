  ...
  created() {
    ...
    bus.$on("close_current_tags", () => {
      for (let i = 0, len = this.tagsList.length; i < len; i++) {
        ...
        if (item.push === this.$route.fullPath) {
          if (i < len - 1) {
...
.tags {
  ...
  background: #fff;
  padding-right: 120px;
  box-shadow: 0 5px 10px #ddd; /** box-shadow都是用于诸如按钮，文本块，某些图标 */
}
...
.tags-li {
  ...
  transition: all 0.3s ease-in; /*  transition 属性用于状态过度效果  ease-in 规定以慢速开始的过渡效果 */
}
